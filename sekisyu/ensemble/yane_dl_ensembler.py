...
class YaneDLEnsembler(BaseEnsembler):
    ...
    def ensemble(self, playinfos: List[BasePlayInfoPack], pos: str) -> BasePlayInfoPack:
        ...
        assert (
            len(playinfos) == 2
        ), f"invalid number of playinfos. detected={len(playinfos)}, expected=2"
        yane_bestmove = playinfos[1].infos[0].pv[0]
        yane_nodes = playinfos[1].infos[0].nodes
        node_best = playinfos[0].infos[0].nodes

        output = from_dict(
            data_class=BasePlayInfoPack, data=dataclasses.asdict(playinfos[0])
        )
...
class YaneDLEnsemblerPositive(BaseEnsembler):
    ...
    def ensemble(self, playinfos: List[BasePlayInfoPack], pos: str) -> BasePlayInfoPack:
        ...
        yane_nodes = playinfos[1].infos[0].nodes
        dl_nodes = (
            playinfos[0].infos[0].nodes
        )  # dlshogi側のnodesは全探索数ではなくbestmoveのnodesでいいのか？
        yane_eval_positive = (
            yane_value + self.bonus_ratio_a * yane_nodes / dl_nodes + self.bonus_ratio_b
        )
